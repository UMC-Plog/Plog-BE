...
public interface ReportRepository extends JpaRepository<Report, Long> {
    ...
    @Query("select report.id as reportId, "
            ...
            + "report.pdfObjectKey as pdfObjectKey, "
            + "report.pdfFileName as pdfFileName "
            + "from Report report "
    ...
    @Query("select report.id as reportId, "
            ...
            + "report.completedAt as completedAt, "
            + "report.createdAt as createdAt, "
            + "result.pdfObjectKey as pdfObjectKey, "
            + "result.pdfFileName as pdfFileName "
            + "from Report report "
            + "join ProjectMember member on member.project = report.project "
            + "and member.user.id = :userId and member.status = :memberStatus "
            + "left join ReportMemberResult result on result.report = report and result.projectMember = member "
            + "order by case when report.completedAt is null then 1 else 0 end, "
            + "report.completedAt desc, report.id desc")
