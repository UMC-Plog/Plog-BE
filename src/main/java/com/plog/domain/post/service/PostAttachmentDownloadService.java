...
public class PostAttachmentDownloadService {
    ...
    public AttachmentDownloadResponse createDownloadUrl(
            Long projectId, Long postAttachmentId, Long userId) {
        ...
        PostAttachment attachment = postAttachmentRepository
                .findWithFileAndProjectById(postAttachmentId)
                .orElseThrow(() -> new ApiException(PostErrorCode.POST_ATTACHMENT_NOT_FOUND));

        // requireActiveMember 는 "이 사람이 projectId 의 멤버인가"만 본다. 첨부가 그 프로젝트
        // 것인지는 안 보므로 여기서 대조하지 않으면 남의 프로젝트 파일을 받을 수 있다.
        Long ownerProjectId = attachment.getPost().getProjectMember().getProject().getId();
        if (!ownerProjectId.equals(projectId)) {
            throw new ApiException(PostErrorCode.POST_ATTACHMENT_NOT_FOUND);
        }

        if (attachment.getAttachmentType() != AttachmentType.FILE) {
            throw new ApiException(PostErrorCode.INVALID_ATTACHMENT_TYPE);
        }
