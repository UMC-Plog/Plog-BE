...
// 태그 설명은 PostControllerDoc 과 같아야 한다. 다르면 springdoc 이 한쪽으로 덮어쓴다.
@Tag(name = "Post", description = "프로젝트 게시글 · 댓글 · 좋아요 (인증 및 활성 멤버 필요)")
public interface PostAttachmentControllerDoc {

    @Operation(
            summary = "게시글 첨부 다운로드 URL 발급",
            description = """
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "400",
                    description = "LINK 첨부에 발급을 요청함 (INVALID_ATTACHMENT_TYPE)",
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "404",
                    description = "첨부가 없거나 해당 프로젝트 소속이 아님 (POST_ATTACHMENT_NOT_FOUND)",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = ApiResponse.class))
            )
    })
    ResponseEntity<ApiResponse<AttachmentDownloadResponse>> createDownloadUrl(
            ...
            @Parameter(description = "게시글 첨부 ID", example = "3") Long postAttachmentId,
            Long userId
    );
}
