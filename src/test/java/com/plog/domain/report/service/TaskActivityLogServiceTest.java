...
class TaskActivityLogServiceTest {
    ...
    @BeforeEach
    void setUp() {
        service = new TaskActivityLogService(activityLogRepository, projectMemberRepository, taskRepository);
    }
    ...
    void 상태변경을_활동_로그로_적재하고_linkedTask를_바로_채운다() {
        ...
        service.collectStatusChanged(1L, 7L, TaskStatus.DONE, occurredAt);
        ...
        assertThat(saved.getMetadata()).isEqualTo("{\"taskId\":1,\"newStatus\":\"DONE\"}");
    ...
    void 첨부_추가를_활동_로그로_적재한다() {
        ...
        assertThat(saved.getContent()).isNull();
        assertThat(saved.getMetadata()).isEqualTo("{\"taskId\":1,\"attachmentId\":9}");
        assertThat(saved.getLinkedTask()).isSameAs(task);
    }
